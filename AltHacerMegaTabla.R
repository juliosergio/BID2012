...
for (cc in cuencas) {
    ...
    # Directorio c/cuenca:
    cdir <- paste0(bdir, "/", cc)
    # Los archivos en cada cuenca que terminan en ".._AltAnomMens.txt":
    files <- system2("ls", paste0(cdir, "/*_AltAnomMens.txt"), stdout=T)
    # Haré tablitas por cuenca
    miniT <- NULL
    for (ff in files) {
        # El nombre del archivo sin el apéndice ".._AltAnomMens.txt"
        bare <- strsplit(ff, "_AltAnomMens." , fixed=T)[[1]][1] # Sin "_AltAnomMens.txt"
